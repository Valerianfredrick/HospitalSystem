...
<head>
    ...
    <title>All Patients — {{ config('app.name', 'MediCore HMS') }}</title>
...
<div class="max-w-5xl mx-auto">
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
            ...
            <h1 class="font-bold text-gray-800 text-lg">All Patients</h1>
        </div>
        <a href="{{ route('receptionist.patients.create') }}"
           class="px-4 py-2 bg-primary-600 text-white text-sm font-semibold rounded-full hover:bg-primary-700 transition-colors">
            ...
            <table class="patient-table">
                ...
                @forelse($patients as $p)
                    <tr>
                        ...
                        <td class="capitalize text-xs">{{ $p->status }}</td>
                        <td>
                            @if($p->doctor)
                                <span class="badge badge-assigned">Dr. {{ $p->doctor->name }}</span>
                            @else
                ...
                @empty
                    <tr><td colspan="5" class="px-6 py-12 text-center text-gray-400">No patients found.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
